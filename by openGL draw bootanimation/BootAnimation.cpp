...
status_t BootAnimation::readyToRun() {
    ...
    // MStar Android Patch Begin
    // set eOpaque to bootanimation
    //Kitkat use ES2.0 as render engine, the alpha blend alogith differ with AN4.3
    //can not set surface eOpaque
...
//MStar Android Patch End
    ...
    ALOGI("Bootanimation EGL informations:");
...
bool BootAnimation::android()
{
    ...
    // MStar Android Patch Begin
    // move boot animation to right-bottom and transparent
...
// MStar Android Patch Begin
// Boot video advert issue, "mstar.videoadvert.finished" is
//  0 means that video advert does not finish
//  1 means that video advert has finished
//  0 is default value
//
// When there has a video advert file to be played, we has cleared screen
// , so users can see video advert behind screen.
// Bootanimation will wait for video advert to finish, before it exits.
// After bootanimation exits, users can see launcher UI.
//
// Return Value
// true means that a video advert file has been played
// false means that no video advert
bool BootAnimation::videoAdvert() {
    char value[PROPERTY_VALUE_MAX];
    int exitnow;
    ...
    // wait the video advert file to finish playing
    do {
        property_get("mstar.videoadvert.finished", value, "0");
        exitnow = atoi(value);
        if (exitnow != 0) {
            break;
        }
